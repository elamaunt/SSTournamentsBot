...
namespace SSTournamentsBot
{
    public static class SystemExtensionMethods
    {
        ...
        public static string PrettyPrint(this TimeSpan time, bool isRussian)
        {
            if (isRussian)
            {
                ...
                if (time.TotalHours < 1.0)
                    return PrintMinutes(time.Minutes, isRussian);

                return $"{PrintHours(time.Hours, isRussian)} {PrintMinutes(time.Minutes, isRussian)}";
            }
            else
            {
                if (time.TotalMinutes < 1.0)
        ...
        private static string PrintMinutes(int minutes, bool isRussian)
        {
            if (isRussian)
            {
                switch (minutes)
        ...
        private static string PrintHours(int hours, bool isRussian)
        {
            if (isRussian)
            {
                switch (hours)
